...
var settings;
var settingsFile; 
var defaultSettingsFile = 'buildjs.json';
...
function main() {
  ...
  settingsFile = buildfx.valueForCommandlineArg('settings') || defaultSettingsFile;
...
function applyDefaultSettings(settings, hasWatchCommandlineArg) {
...
function getDependencies(fileName, dependencyRegex) {
  var d = [];
  var dependencyRegex = dependencyRegex || /\/\/\s*@depend\s*([\S]*)/g;
  var fileText = fs.readFileSync(fileName, 'utf8'); 
  var match = dependencyRegex.exec(fileText);

  while (match) {
    d.push(match[1]);
    match = dependencyRegex.exec(fileText);      
  }
